...
## 7. `vmr_to_actual_audio_ms`(唇音同步)

定义 `actualAudioStart − firstVmr1Time`(负=声音先,正=口型先)。

### 7.1 数据(20.4,最近 3h,n=568)
avg **92ms**,范围 43~160,**全部为正**(口型先);符号分布:口型先 0~100ms(339)、100~200ms(229)、声音先 **0** 条。

### 7.2 结论:音画同步健康
- **方向对**:永远"先动嘴后出声"(0 条声音抢跑)。人耳对"声音早于口型"更敏感,嘴先动是更自然的方向。
- **量级可接受**:按广播 A/V 同步经验(声音滞后口型 ≤ ~125ms 不易察觉),~92ms 属可接受区。
- **含 ~20~50ms RMS 检测滞后**,真实嘴领先约 ~60ms(更好);RMS 轮询 50→20ms 已减小该偏差。
- 与 `lip_sync_diff`(结尾音频比嘴动长 ~185ms)互补:两端偏差都小,口型-声音贴合良好。
- **质量确认型指标,无需改动。**

---

## 8. 修复后运行观察(557 轮 / ~140 分钟)

回退(`f9250d7`)生效于 20:17,至 22:37 约 **140 分钟、557 个干净轮次,无截断复发**。窗口内各指标 avg / 范围:

| 指标 | avg | 范围 |
|------|----:|------|
| Click->Open | 241 | 147~711 |
| Click->VD | 243 | 149~714 |
| 1st ASR | 3798 | 3774~3863(超稳) |
| ASR Tail | 435 | 408~496 |
| Wait TTS | 1359 | 1192~2883 |
| TTS->Lip | 746 | 434~1624 |
| Wait Play | 2198 | 2005~3707 |
| Play Dur | 20129 | 12340~23041 |
| Avatar Dur | 20230 | 12870~23208 |
| Lip Sync | 92 | 43~160 |

观察:
- **1st ASR 超稳(跨度仅 89ms)**——注入实时化 + tAudioStart 锚首帧见效(修前 4092~4658 乱跳)。
- **回复内容变长**:从早先 ~13s 涨到 ~18~22s(近期在 ~18.6s/~21.7s 两变体间交替),属服务端 LLM 内容变化。avatar_dur 分桶:21~22.5s 有 296 条(主群,未在硬顶堆积),>22.5s 仅 4 条。
- **完成超时余量变薄**:回复变长后 25s 超时(数字人 ~2s 开口 → 上限 avatar_dur ≈23s)只剩 ~2~3s 余量。**据此把完成超时 25s→35s** 恢复安全余量。
  - **代价(诚实)**:回退后完成判定是**固定等到超时**(无可靠"回复结束"信号,故无提前退出),所以 **每轮都会等满 35s**,即每轮 cadence 比之前 +10s。这是"正确性(不截断)优先于速度"的权衡;待拿到新协议的"回复结束"标志后,才可能安全地提前退出。

## 9. 端到端总览(各段 avg 占比,n=234)

`total_interaction_ms` avg = **26496ms**,非重叠拆解(tClick → 回复播放结束):

| 段 | avg (ms) | 占比 | 类型 |
|----|--------:|-----:|------|
| Click→VD 建连 | 240 | 0.9% | 开销 |
| 建连→开播(含 800ms 固定 sleep+setup) | ~900 | 3.4% | 开销 |
| 播放测试音频(用户"说话") | 3326 | 12.6% | 内容 |
| 说完→听见回复(Wait Play) | 2200 | 8.3% | **等待** |
| 回复播放(Play Dur) | 19830 | 74.8% | 内容 |
| 合计 | 26496 | 100% | |

- **内容(非延迟)占 87.4%**(说话 3.3s + 回复播放 19.8s),不是卡顿;真正开销/等待仅 **12.6%(~3.3s)**。
- **感知等待 Wait Play 2200ms 再拆**:ASR 收尾 435(20%)+ **LLM 生成 926(42%)** + **TTS 合成下发 839(38%)**。用户干等的 80% 是 LLM+TTS,全在服务端。

## 小结:整个交互指标系列
- **上行**:建连 ~240ms(3 RTT,详见网络文档)、ASR 收尾 ~435ms;`1st ASR` 因非流式 ≈ 音频时长 + 尾延迟。
- **下行(感知等待 ~2.2s)**:LLM(~926ms)+ TTS 合成下发(~839ms)主导,均服务端;插件无可优化,杠杆在服务端(流式 TTS、更快 LLM)。
- **播放/唇形**:回复 ~13→~20s(内容变长)、音画同步健康(嘴领先 ~60~92ms、结尾音频长 ~185ms)。
- **插件侧产出**:事件驱动打点(去 100ms 轮询)+ 拆 `click_to_vd_open_ms`;ASR end 延时 300→100ms;注入实时化(绝对调度 + tAudioStart 锚首帧)+ `audio_inject_ms`;RMS 轮询 50→20ms;移除坏死 `tts_total_duration_ms`;完成超时 25→35s(应对回复变长)。
- **教训**:完成判定的提速尝试(3s 静默窗口)被实测否决——多段回复段间 >3s,会截断;在拿到可靠"回复结束"信号前,固定超时是正确选择。
