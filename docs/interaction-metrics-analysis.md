...
## 小结:下行/播放/唇形一组
- 感知等待 `Wait Play` ~2.1s = ASR(430)+ LLM(870)+ TTS 合成下发(790),全在服务端;
- 数字人回复 ~13s、`tts_to_lip` ~730ms(首帧口型)、音画同步健康(嘴领先 ~60~92ms、结尾音频长 ~185ms);
- 插件侧产出:移除坏死 `tts_total_duration_ms`、RMS 轮询 50→20ms;完成判定的提速尝试被实测否决(多段回复需 25s 余量)。
