...
> **重要更正(相对初版)**:初版把握手拆成"~90ms 网络 + ~57ms 服务端 TLS"。
> 这里要澄清:那 ~58ms **确实是 TLS 握手耗时**(`appconnect − connect` 量的就是它),
> 但它**既不是网络、也不是华为服务端,而是测量用的那台老客户端自身的开销**——利雅得侧探测
> 用的是腾讯 CentOS 7 的 OpenSSL 1.0.2(被迫 TLS 1.2、RSA/老代码路径慢)。
> 判据:**同一台华为服务器**,换迪拜 macOS(TLS 1.3)去测,TLS 段就是 ~50ms 且**恰等于 1 个网络 RTT**;
> 若华为服务端真花 57ms,迪拜应测出 50+57≈107ms,实测只有 ~50ms → 华为端做 TLS 并没花这 57ms。
> 真实生产客户端是 **Chrome(TLS 1.3)**,握手行为同 macOS,**不产生这 57ms**。故它是**测量客户端伪影**,
> 不应计入迪拜→华为握手预算:**~150ms 全新握手 ≈ 3 个网络 RTT**,纯网络往返。见 §4.3、§5。
