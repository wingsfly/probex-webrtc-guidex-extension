...
在一台**网络良好**的客户端(instance 523)上,用脚本在 voiceDictation WS **打开后 ~3s(音频中途、收到最终结果 status=2 之前)强制关闭该 WS**(`ws.close(4000)`),以此模拟"弱网把连接断掉"。结果:

```
健康(不干预):   create → OPEN(452ms) → recv status=1 → status=2(6.3s) → CLOSE 1000 clean → 下一轮开新 WS ✓
复现(中途断连):  create → OPEN(476ms) → CLOSE 4000(3.06s,无 status=2) → ✗ 之后不再开任何新的 voiceDictation WS
```

- 强制断连后 **~1.5 分钟内应用没有再创建任何新的 voiceDictation 会话**,后续每次触发都失败——**与一体机的现象完全一致**(触发后无新会话、字段全空)。
- 页面需 **刷新** 才恢复。

**由此坐实根因**:voiceDictation WS 在收到最终结果(`status=2`)**之前异常关闭**(弱网丢连即此情形),会使 GuideX 应用**无法开启新会话、卡在拾音态**。健康路径下 WS 是 `status=2` 后 `code=1000` 干净关闭,而异常关闭(非 1000、或中途关闭)后应用状态未复位。

## 5. 在真实弱网客户端上抓取(可选,进一步佐证)
