...
(function () {
        ...
        body: JSON.stringify({
          ...
          output_schema: {
            ...
            extra_fields: [
              ...
              { name: 'video_frames_decoded', type: 'number', unit: 'frames', chartable: true, default_hidden: true },
              { name: 'video_frames_dropped', type: 'number', unit: 'frames', chartable: true, default_hidden: true },
              { name: 'video_fps', type: 'number', unit: 'fps', chartable: true },
              { name: 'quality_limitation', type: 'string', description: 'cpu/bandwidth/none' },
              { name: 'available_outgoing_bitrate', type: 'number', unit: 'bps', chartable: true, default_hidden: true },
              ...
              { name: 'connection_count', type: 'number', description: 'Active PeerConnection count', default_hidden: true },
            ],
          },
        }),
      });
  ...
  async function autoTestCycle() {
    ...
    const metrics = {
      ...
      audio_duration_ms: Math.round(audioBuffer.duration * 1000),
      audio_inject_ms: Math.round(tAudioEnd - tAudioStart),
